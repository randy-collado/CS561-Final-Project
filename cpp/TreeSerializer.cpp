...
#define align(p, a) (((long)(p) + (a - 1)) & ~(a - 1))
	...
	S_Node* TreeSerializer::readNode(){
        if (at_eof){
            std::cerr << "[ERROR]: Could not read\n[MESSAGE]: EOF has been reached" << std::endl;
            return nullptr;
        }
...
//		std::cout << "Bytes read: " << bytes << std::endl;
    ...
    /*
     * The parameters of this function should be a multiple of 512, the size of a node.
     * Will return nullptr if the offset is not 512-byte aligned.
     *
     * Might be useful to allow this to return pointers to individual members of a struct, but casting might be needed
     * in this case
     */
    ...
    void TreeSerializer::write_offset_metadata(tier_offsets *metadata) {
        ...
        if(bytes<=0) std::cout << "[ERROR]: Could not write metadata\n[MESSAGE]: " + std::string(strerror(errno))<<std::endl;
    }
